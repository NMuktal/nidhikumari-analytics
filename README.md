# Hi, I'm Nidhi Kumari 👋

📍 Texas, United States  
🎯 IT Business Analyst | Business Intelligence | Data Analytics  | Business Intelligence Engineer
📧 Email: Nidhi.kumari.muktal@gmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/nidhi-kumari-486102b2/

---

## 👩‍💼 About Me
I am a results-driven **IT Business Analyst** with 6+ years of experience delivering **data-driven insights, BI solutions, and analytics platforms** across finance, technology, and enterprise systems. I specialize in transforming complex business requirements into scalable dashboards, ETL pipelines, and actionable insights.

---

## 💼 Professional Experience Highlights
- Designed **end-to-end BI solutions** using **NetSuite, NSAW, SQL**
- Built **ETL pipelines** using star schema models for enterprise reporting
- Delivered dashboards using **Power BI, Tableau, Excel, SQL**
- Developed **predictive models** using Python & ML techniques
- Improved operational efficiency through **JIRA & ClickUp**
- Automated reporting & validation for large-scale datasets (300K+ records)

---

## 🛠️ Technical Skills

**Data & BI Tools**
- SQL, Tableau, Power BI, Excel (Macros, Pivot Tables)
- NetSuite, NSAW, Oracle, Teradata, SQL Server

**Programming & Analytics**
- Python, R
- Predictive Modeling, Forecasting, Regression, A/B Testing
- EDA, Feature Engineering, Clustering, Time Series

**Cloud & Project Tools**
- AWS, Azure, GCP
- JIRA, ClickUp

---

## 📊 Featured Projects (Coming Soon 🚀)
✔ BI Dashboards (Power BI / Tableau)  
✔ SQL Data Analysis Projects  
✔ Python Predictive Analytics  
✔ ETL & Data Modeling Examples  

---

## 🎓 Education & Certifications
- **M.S. Business Analytics** – University of Texas at Dallas  
- **M.Sc. Operational Research** – University of Delhi  
- **B.Sc. Mathermatics** – University of Delhi  

**Certifications**
- Certified Scrum Master (CSM)
- Oracle Cloud Infrastructure 2024
- Certified AI Foundation Associate

---

## 📫 Let's Connect!
Feel free to reach out via email or LinkedIn. I’m open to collaboration and analytics projects!
